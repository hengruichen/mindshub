...
if __name__ == '__main__':
    ...
    if telemetry_file_exists(config['paths']['telemetry']):
        disable_telemetry(config['paths']['telemetry'])

    if config.get("jobs", {}).get("disable") is not True:
        api_arr = ['http', 'mysql']
    elif args.api is None:  # If "--api" option is not specified, start the default APIs
        api_arr = ['http', 'mysql']
